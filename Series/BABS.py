...
#
# Better ABsolute Scanner based on default scanner code from PMS 0.9.3.5 for Ubuntu 2011-10-15 by jmjf (on Plex Forums)
# Source: https://forums.plex.tv/index.php/topic/31081-better-absolute-scanner-babs/

# Does a better job of dealing with absolute numbered files and addresses the problem of series with numbers in the the name.
# Place this file in /var/lib/plexmediaserver/Library/Application Support/Plex Media Server/Scanners/Series/var/lib/plexmediaserver is a.k.a. ~/ to the plex user
# you may have to create the Series subdirectory
# This code has only been tested on Ubuntu 10.4.2 with PMS 0.9.3.5 and my media collection. Use at your own risk. That said, I expect it will work on other platforms.
# PS: Am running it on Synology hacked box
# all debug messages are left in -- will only show on console -- include BABS: at beginning of line. all changes except debug messages are called out with comments begining # BABS -- 
...
def Scan(path, files, mediaList, subdirs, language=None, root=None):
  ...
  print "BABS: Scan"
  VideoFiles.Scan(path, files, mediaList, subdirs, root)                                                  # Scan for video files.
  paths = Utils.SplitPath(path)                                                                           # Take top two as show/season, but require at least the top one.
  if len(paths) == 1 and len(paths[0]) == 0:
    print "BABS: len(paths) == 1 and len(paths[0]) == 0"
    for i in files:  file = os.path.basename(i)                                                           # Run the select regexps we allow at the top level.
    ...
    if len(files) == 1:
      for rx in standalone_episode_regexs:
        ...
        if len(res):
          ...
          episode = int(episode)
          if len(endEpisode) > 0:  endEpisode = int(endEpisode)
          else:                    endEpisode = episode
          for ep in range(episode, endEpisode+1):
    ...
    if done == False:
      ...
      for rx in ends_with_episode:  show = re.sub(rx, '', show)
      ...
      for i in files:
        ...
        if ext.lower() in ['.mp4', '.m4v', '.mov']:
          ...
          try: 
            ...
            # Episode.
            m4ep = None
            try:      m4ep = int(find_data(mp4fileTags, 'moov/udta/meta/ilst/tracknum'    )) # tracknum (can be 101)
            except: 
              try:    m4ep =     find_data(mp4fileTags, 'moov/udta/meta/ilst/tvepisodenum')  # tvepisodenum (can be S2E16)
              except: m4ep = int(find_data(mp4fileTags, 'moov/udta/meta/ilst/tvepisode'   )) # TV Episode (can be 101)
              ...
              try:
                ...
                for rx in episode_regexps[:-1]:
                  ...
                  if match:
                    ...
                    found = True
                if found == False and re.match('[0-9]+', str(m4ep)):  m4ep = int(m4ep) % 100# Carefully convert to episode number.
                elif found == False:                                  m4ep = int(re.findall('[0-9]+', m4ep)[0])
              except: pass
        ...
        if done == False:
          cleanName, cleanYear = VideoFiles.CleanName(file) # Take the year out, because it's not going to help at this point.
          ...
          for rx in whackRx:  file = re.sub(rx, ' ', file)
          ...
          for rx in episode_regexps:
            ...
            if match:
              ...
              episode = int(match.group('ep'))
              endEpisode = episode
              if match.groupdict().has_key('secondEp') and match.group('secondEp'):  endEpisode = int(match.group('secondEp'))
              ...
              if rx == episode_regexps[-1]:
                ...
                if the_season == 0:  break  # Skip episode 0 on the weak regex since it's pretty much never right.
                  
                # Make sure this isn't absolute order.
                if seasonNumber is not None:
                  if seasonNumber != the_season: # Something is amiss, see if it starts with an episode numbers.
                    if re.search('^[0-9]+ -', file):  break # Let the episode matcher have it.
                    
                    # Treat the whole thing as an episode.
                    episode = episode + the_season*100
                    if endEpisode is not None:  endEpisode = endEpisode + the_season*100
        ...
        if done == False:  print "Got nothing for:", file
